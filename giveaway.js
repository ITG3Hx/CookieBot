...
async function endGiveaway(gw) {
  ...
  pushActivity("giveaway", `"${gw.prize}" ended: ${gw.entrants.length} entries, ${winners.length} winner(s)`);
...
async function handleGiveawayCommand(interaction) {
  ...
  if (sub === "start") {
    ...
    scheduleEnd(gw);
    pushActivity("giveaway", `"${prize}" started by ${interaction.user.tag} in #${channel.name}`);
...
function gwView(g) {
  return {
    messageId: g.messageId, channelId: g.channelId, guildId: g.guildId,
    prize: g.prize, winners: g.winners, hostId: g.hostId, requiredRoleId: g.requiredRoleId,
    createdAt: g.createdAt, endsAt: g.endsAt, ended: !!g.ended,
    entries: g.entrants.length, winnerIds: g.winnerIds || [],
  };
}

function webListGiveaways() {
  const all = Object.values(giveaways);
  return {
    active: all.filter(g => !g.ended).sort((a, b) => a.endsAt - b.endsAt).map(gwView),
    ended: all.filter(g => g.ended).sort((a, b) => b.endsAt - a.endsAt).slice(0, 25).map(gwView),
  };
}

async function webStartGiveaway({ channelId, prize, duration, winners, requiredRoleId }) {
  const ms = parseDuration(duration);
  if (!ms) return { error: "Invalid duration, use e.g. 30m, 2h, 1d (min 10s, max 14d)" };
  const cleanPrize = String(prize || "").trim().slice(0, 240);
  if (!cleanPrize) return { error: "Prize is empty" };
  const n = Math.min(20, Math.max(1, parseInt(winners, 10) || 1));
  const channel = await client.channels.fetch(channelId).catch(() => null);
  if (!channel || !channel.isTextBased()) return { error: "Channel not found or not a text channel" };

  const gw = {
    messageId: null, channelId: channel.id, guildId: channel.guildId,
    prize: cleanPrize, winners: n, hostId: client.user.id,
    requiredRoleId: (requiredRoleId && /^\d{5,25}$/.test(requiredRoleId)) ? requiredRoleId : null,
    createdAt: Date.now(), endsAt: Date.now() + ms, entrants: [], winnerIds: [], ended: false,
  };
  let msg;
  try { msg = await channel.send({ embeds: [gwEmbed(gw)], components: [entryRow(0)] }); }
  catch (e) { return { error: `Could not post in that channel: ${e.message}` }; }
  gw.messageId = msg.id;
  giveaways[msg.id] = gw; save();
  scheduleEnd(gw);
  pushActivity("giveaway", `"${cleanPrize}" started via dashboard in #${channel.name}`);
  return { giveaway: gwView(gw) };
}

async function webEndGiveaway(messageId) {
  const gw = giveaways[String(messageId).trim()];
  if (!gw) return { error: "No giveaway with that message ID" };
  if (gw.ended) return { error: "That giveaway already ended" };
  const winners = await endGiveaway(gw);
  return { giveaway: gwView(gw), winners };
}

async function webRerollGiveaway(messageId, n) {
  const gw = giveaways[String(messageId).trim()];
  if (!gw) return { error: "No giveaway with that message ID" };
  if (!gw.ended) return { error: "End the giveaway first" };
  if (!gw.entrants.length) return { error: "That giveaway had no entries" };
  const pool = gw.entrants.filter(e => !(gw.winnerIds || []).includes(e));
  if (!pool.length) return { error: "Everyone who entered has already won" };
  const winners = pickWinners(pool, Math.min(20, Math.max(1, parseInt(n, 10) || 1)));
  gw.winnerIds = [...(gw.winnerIds || []), ...winners]; save();
  const { channel } = await fetchMessage(gw);
  if (channel) await channel.send(`${EMOJI} **Reroll!** New winner(s) for **${gw.prize}**: ${winners.map(w => `<@${w}>`).join(", ")}!\nContact the host <@${gw.hostId}> to claim.`).catch(() => {});
  pushActivity("giveaway", `"${gw.prize}" rerolled via dashboard: ${winners.length} new winner(s)`);
  return { giveaway: gwView(gw), winners };
}
...
module.exports = {
  initGiveaways, handleGiveawayCommand, GIVEAWAY_COMMANDS,
  webListGiveaways, webStartGiveaway, webEndGiveaway, webRerollGiveaway,
};
