...
function gwEmbed(gw, { ended = false, winners = null } = {}) {
  ...
  const e = new EmbedBuilder().setColor(COLOR).setTitle(`${EMOJI}  GIVEAWAY  ${EMOJI}`).setDescription(`**${gw.prize}**`);
  if (ended) {
    e.addFields(
...
async function endGiveaway(gw) {
  ...
  if (channel) {
    ...
    if (winners.length)
      await channel.send(`${EMOJI} Congratulations ${winners.map(id => `<@${id}>`).join(", ")}! You won **${gw.prize}**! ${link}`).catch(() => {});
    else
...
async function handleGiveawayCommand(interaction) {
  ...
  if (sub === "start") {
    ...
    const gw = {
      ...
      createdAt: Date.now(), endsAt: Date.now() + ms, entrants: [], ended: false,
    };
  ...
  if (sub === "reroll") {
    ...
    const winners = pickWinners(gw.entrants, n);
    const { channel } = await fetchMessage(gw);
    if (channel) await channel.send(`${EMOJI} **Reroll!** New winner(s) for **${gw.prize}**: ${winners.map(w => `<@${w}>`).join(", ")}!`).catch(() => {});
