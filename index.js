...
async function registerCookieCommands() {
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      console.log(`[CookieBot] Registered ${COOKIE_COMMANDS.length} commands in ${guild.name}: ${COOKIE_COMMANDS.map(c => c.name).join(", ")}`);
    } catch (err) {
      console.error(`[CookieBot] Registration failed for ${guild.name}:`, err.message);
      if (err.rawError) console.error("[CookieBot] Discord detail:", JSON.stringify(err.rawError, null, 2));
    }
  }
}
