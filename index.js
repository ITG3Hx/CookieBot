...
async function resolveTrack(input) {
  // YouTube video link
  const ytValidate = await playdl.yt_validate(input).catch(() => false);
  ...
  if (!results.length) {
    throw new Error("No results found. Try a YouTube link or a search term.");
  }
...
async function playNextTrack(guildId) {
  ...
  if (!queue.tracks.length) {
    ...
    // Disconnect 30 seconds after queue empties
    setTimeout(() => {
      const q = musicQueues.get(guildId);
      if (q && !q.currentTrack && !q.tracks.length && q.connection) {
  ...
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      try {
        ...
        // One of the existing commands may be invalid (e.g. bad description),
        // which makes Discord reject the whole batch. Retry with ONLY cookie commands
        // so our commands still show up.
        console.error(`Merge failed for ${guild.name}, retrying with CookieBot commands only:`, mergeError.message);
        if (mergeError.rawError) console.error(JSON.stringify(mergeError.rawError, null, 2));
        try {
          await guild.commands.set([...COOKIE_COMMANDS]);
          console.log(`CookieBot guild commands registered (cookie-only) for ${guild.name}.`);
        } catch (fallbackError) {
          console.error(`Fallback registration failed for ${guild.name}:`, fallbackError.message);
          if (fallbackError.rawError) console.error(JSON.stringify(fallbackError.rawError, null, 2));
        }
      }
    } catch (error) {
