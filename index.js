...
async function registerCookieCommands() {
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      for (const cmd of COOKIE_COMMANDS) {
        try {
          const found = byName.get(cmd.name);
          if (found) {
            await guild.commands.edit(found.id, cmd);
            console.log(`[CookieBot] /${cmd.name} updated in ${guild.name}`);
          } else {
            await guild.commands.create(cmd);
            console.log(`[CookieBot] /${cmd.name} created in ${guild.name}`);
          }
        } catch (cmdErr) {
          console.error(`[CookieBot] /${cmd.name} FAILED in ${guild.name}:`, cmdErr.message);
          if (cmdErr.rawError) {
            console.error(`[CookieBot] Discord error:`, JSON.stringify(cmdErr.rawError, null, 2));
          }
        }
      }

      console.log(`[CookieBot] Command registration done for ${guild.name}`);
    } catch (err) {
      console.error(`[CookieBot] Could not fetch commands for ${guild.name}:`, err.message);
    }
  }
}
