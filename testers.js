...
// ── Web dashboard API ─────────────────────────────────────────────────────────
function webListTesters() {
  return Object.values(links).map(l => ({
    code: l.code, mcName: l.mcName, discordId: l.discordId || null, discordTag: l.discordTag || null,
    rank: l.rank || null, verified: !!l.verified, createdAt: l.createdAt || null, linkedAt: l.linkedAt || null,
  })).sort((a, b) => (b.linkedAt || b.createdAt || 0) - (a.linkedAt || a.createdAt || 0));
}

function webUnlinkTester(code) {
  const entry = byCode(code);
  if (!entry) return { error: "No link with that code" };
  delete links[entry.code];
  save();
...
module.exports = {
  initTesters, handleTesterCommand, mountTesterApi, TESTER_COMMANDS,
  webListTesters, webUnlinkTester,
};
